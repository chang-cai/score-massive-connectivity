...
class CondRefineNetDilated(nn.Module):
    ...
    def forward(self, x):
        y = torch.randint(0, 1, (x.shape[0],), device=x.device)
        x = x.reshape(x.shape[0], self.config.data.channels, self.config.data.image_size, self.config.data.image_size)
...
class LowRankCondRefineNetDilated(nn.Module):
    def __init__(self, config):
        super().__init__()
        # self.norm = ConditionalInstanceNorm2d
        self.norm = ConditionalInstanceNorm2dPlus
        ...
        self.end_conv = nn.Conv2d(ngf, config.data.channels * self.rank, 3, stride=1, padding=1)
...
class LowRankS2(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.rank = config.rank
        self.low_rank_model = LowRankCondRefineNetDilated(config)
        self.dim = config.data.channels * config.data.image_size * config.data.image_size
        self.diagonal = CondRefineNetDilated(config)

    def forward(self, X):
        # cov_factor = self.low_rank_model(X).reshape(X.shape[0], self.dim, self.rank) # previous checkpoint
        cov_factor = self.low_rank_model(X).reshape(X.shape[0], self.rank, self.dim).permute(0, 2, 1)

        cov_product = torch.matmul(cov_factor, cov_factor.permute(0, 2, 1))  # batch, dim, dim
        cov_diag = self.diagonal(X)  # batch, dim
